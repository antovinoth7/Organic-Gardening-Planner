...
export const exportBackupWithImages = async (): Promise<string> => {
  try {
    ...
    throw new Error('Failed to export backup with images: ' + (error as Error).message);
  }
};
 ...
 */
export const importBackupWithImages = async (overwrite: boolean = false): Promise<{
  plants: number;
  tasks: number;
  taskLogs: number;
  journal: number;
  ...
  try {
    ...
    const updatedJournal = backup.journal.map(entry => {
      if (entry.photo_urls && entry.photo_urls.length > 0) {
        const updatedPhotos = entry.photo_urls.map(photoUri => {
          ...
          return filename && imageUris.has(filename) ? imageUris.get(filename)! : photoUri;
        });
      ...
      if (entry.photo_url) {
        ...
        const newUri = filename && imageUris.has(filename) ? imageUris.get(filename)! : entry.photo_url;
    ...
    if (overwrite) {
      ...
      const mergedTasks = mergeByIdPreferBackup(existingTasks, backup.tasks);
      const mergedLogs = mergeByIdPreferBackup(existingLogs, backup.taskLogs);
      const mergedJournal = mergeByIdPreferBackup(existingJournal, updatedJournal);
    ...
    throw new Error('Failed to import backup with images: ' + (error as Error).message);
  }
};
...
export const importImagesOnly = async (): Promise<number> => {
  try {
    ...
    console.log(`Imported ${imageUris.size} images`);
    
    // Note: Images are saved with their original filenames
    // The app will automatically use them when displaying plants/journals
    // because the filenames match the stored URIs
